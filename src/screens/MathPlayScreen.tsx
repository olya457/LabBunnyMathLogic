  ...
  return (
    <ImageBackground source={BG} style={styles.bg} resizeMode="cover">
      <SafeAreaView style={styles.safe}>
        <View style={[styles.topBar, { paddingTop: Math.max(10, insets.top + 8), paddingHorizontal: 14 }]}>
        ...
        <View style={styles.mid}>
          ...
          >
            {task.lines.map((t, idx) => (
              <Text key={idx} style={styles.cardLine}>{t}</Text>
            ))}
            ...
            {task.options.map((opt, i) => {
              ...
              return (
                <Pressable
                  ...
                  disabled={locked}
                  onPress={() => onPick(i)}
                  style={[styles.ansBtn, { width: btnW, height: btnH, backgroundColor: bgColor }]}
                >
        ...
        <Animated.View style={[styles.bunnyWrap, { 
          paddingBottom: Math.max(10, insets.bottom + 8),
          transform: [
            { translateY: bunnyAnim.interpolate({ inputRange: [0, 1], outputRange: [10, 0] }) },
            { scale: bunnyAnim.interpolate({ inputRange: [0, 1], outputRange: [0.98, 1] }) }
          ]
        }]}>
          ...
          <View style={styles.modalBg}>
            <View style={styles.modalCard}>
              ...
              <View style={styles.rewardRow}>
                ...
                <Text style={styles.rewardTxt}>+{currentLevelData.tasks.length * REWARD_CORRECT}</Text>
              </View>
...
function buildLevels(): Level[] {
  ...
  return [
    { level: 1, title: 'L1', tasks: [
      { id: '1-1', lines: ['🍪 + 🍪 = ?'], questionLine: 'Choose:', options: toOpts([2, 3, 4, 1]), correctIndex: 0 },
    ...
    { level: 10, title: 'L10', tasks: [
      { id: '10-1', lines: ['🍪 × 3 = 27'], questionLine: '🍪 = ?', options: toOpts([8, 9, 7, 6]), correctIndex: 1 },
      { id: '10-2', lines: ['⭐ × 2 + 🟦 = 22', '⭐ = 7, 🟦 = 8'], questionLine: 'Correct?', options: toOpts([1, 0, 2, 3]), correctIndex: 0 },
...
const styles = StyleSheet.create({
  ...
  topBtn: { width: 38, height: 38, borderRadius: 10, backgroundColor: 'rgba(70, 120, 70, 0.4)', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)' },
  topBtnTxt: { color: '#FFF', fontSize: 24, fontWeight: '900', marginTop: -2 },
  topTitlePill: { flex: 1, height: 38, borderRadius: 10, backgroundColor: 'rgba(70, 120, 70, 0.4)', alignItems: 'center', justifyContent: 'center' },
  topTitleTxt: { color: '#FFF', fontWeight: '900', fontSize: 16 },
  topCoinsPill: { height: 38, paddingHorizontal: 12, borderRadius: 10, backgroundColor: 'rgba(70, 120, 70, 0.4)', flexDirection: 'row', alignItems: 'center', gap: 6 },
  ...
  card: { borderRadius: 15, backgroundColor: '#D8F6FF', borderWidth: 2, borderColor: 'rgba(0,0,0,0.1)', alignItems: 'center', justifyContent: 'center', padding: 20 },
  ...
  ansBtn: { borderRadius: 25, alignItems: 'center', justifyContent: 'center', borderWidth: 1.5, borderColor: 'rgba(0,0,0,0.1)' },
  ...
  shareBtn: { position: 'absolute', left: 0, bottom: 50, width: 40, height: 40, borderRadius: 10, backgroundColor: '#2B6DE3', alignItems: 'center', justifyContent: 'center' },
  shareTxt: { color: '#FFF', fontWeight: '900', fontSize: 18 },
  primaryBtn: { width: '100%', height: 44, borderRadius: 12, backgroundColor: '#2B6DE3', alignItems: 'center', justifyContent: 'center' },
  primaryTxt: { color: '#FFF', fontWeight: '900', fontSize: 16 },
  secondaryBtn: { width: '100%', height: 40, borderRadius: 12, borderWidth: 1, borderColor: '#2B6DE3', alignItems: 'center', justifyContent: 'center' },
